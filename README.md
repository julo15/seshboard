...
| cmux | Full | AppleScript focus (matches `id of tab` against the workspace UUID captured from `$CMUX_WORKSPACE_ID` in the session-start hook); AppleScript resume via `new tab` + `input text` |
