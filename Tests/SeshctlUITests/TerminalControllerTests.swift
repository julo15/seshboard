...
// MARK: - Cmux Tree JSON Helpers

@Suite("TerminalController - findPaneIdInTree")
struct FindPaneIdInTreeTests {
    ...
    func firstPane() {
        #expect(TerminalController.findPaneIdInTree(json: Self.json, surfaceId: "S2") == "P1")
    }

    @Test("Returns pane_id for surface in second pane")
    func secondPane() {
        #expect(TerminalController.findPaneIdInTree(json: Self.json, surfaceId: "S3") == "P2")
    }

    @Test("Returns nil when surface is not present")
    func missingSurface() {
        #expect(TerminalController.findPaneIdInTree(json: Self.json, surfaceId: "S99") == nil)
    }

    @Test("Returns nil for malformed JSON")
    func malformedJSON() {
        #expect(TerminalController.findPaneIdInTree(json: "{not json", surfaceId: "S1") == nil)
    }
    ...
    func multiWindow() {
        let json = """
            ...
            """
        #expect(TerminalController.findPaneIdInTree(json: json, surfaceId: "X") == "PX")
    }
}

@Suite("TerminalController - surfaceIds(inPane:)")
struct SurfaceIdsInPaneTests {
    private static let json = """
        {
          "windows": [{
            "workspaces": [{
    ...
    func targetPane() {
        #expect(TerminalController.surfaceIds(inPane: "P1", treeJSON: Self.json) == ["A", "B"])
    }

    @Test("Returns single id for pane with one surface")
    func singleSurface() {
        #expect(TerminalController.surfaceIds(inPane: "P2", treeJSON: Self.json) == ["C"])
    }

    @Test("Returns empty for unknown pane")
    func unknownPane() {
        #expect(TerminalController.surfaceIds(inPane: "P99", treeJSON: Self.json) == [])
    }

    @Test("Returns empty for malformed JSON")
    func malformedJSON() {
        #expect(TerminalController.surfaceIds(inPane: "P1", treeJSON: "garbage") == [])
    }
}
