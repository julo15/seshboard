...
Review-gate refinements: the cmux resume branch now reuses `SessionAction.compoundShellCommand` instead of reimplementing POSIX single-quote escaping. `SystemEnvironment.runShellCommand` reverted to `Void` (the `@discardableResult -> Bool` expansion was CLI-control prototype residue with no production consumer). Empty-directory branch + its test removed from `buildResumeScript` (already guarded upstream by `TerminalController.resume`'s `fileExists` check, and `compoundShellCommand` handles empty-dir internally). Added a code comment above the `|` split in `buildFocusScript` recording the pack-format choice. README notes the macOS Automation TCC prompt users will see on first focus of any AppleScript-driven terminal.
